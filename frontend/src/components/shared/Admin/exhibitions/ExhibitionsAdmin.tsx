...
import { deleteExhibition, getAllExhibitions, moderateExhibition, type Exhibition } from '../../../../api/exhibitions/main.api';
...
export const ExhibitionsAdmin = () => {
    ...
    const handleDelete = async (id: number, title: string) => {
        ...
        if (confirmed) {
            try {
                ...
                showNotification("Выставка удалена", "success");
                loadData();
            } catch (error) {
    ...
    const columns = [
        ...
        { key: 'date', header: 'Дата', render: (item: Exhibition) => formatDate(item.date) },
        {
            key: 'moderate',
            header: 'Статус',
    ...
    const actions = (item: Exhibition) => (
        <>
            <button className="admin-table__view" onClick={() => { setSelectedExhibition(item); setViewModalOpen(true); }}>👁️</button>
    ...
    return (
        <div className="admin-page">
            <div className="admin-page__header"><h1>🏛️ Управление выставками</h1></div>
            <AdminTable data={data} columns={columns} actions={actions} emptyMessage="Выставки не найдены" />

            <AdminModal isOpen={viewModalOpen} onClose={() => { setViewModalOpen(false); setSelectedExhibition(null); }} title={selectedExhibition?.title || ''}>
                {selectedExhibition && (
                    <div className="admin-view">
                        ...
                        <div className="admin-view__grid">
                            <div className="admin-view__item">
                                ...
                                <span className="admin-view__value">{selectedExhibition.id}</span>
                            </div>
                            <div className="admin-view__item">
                                <span className="admin-view__label">Название</span>
                                <span className="admin-view__value">{selectedExhibition.title}</span>
                            </div>
                            <div className="admin-view__item">
                            ...
                            <div className="admin-view__item">
                                <span className="admin-view__label">Посетителей</span>
                                <span className="admin-view__value">{selectedExhibition.visitors_count || 0}</span>
                            </div>
                        </div>
                            ...
                            {exhibitionToReject.image_path && (
                                <div className="admin-reject__field">
                                    ...
                                    <div className="admin-reject__field-value">
                                        <img src={exhibitionToReject.image_path} alt={exhibitionToReject.title} className="admin-reject__image-preview" />
                                    </div>
                                </div>
                            )}
                        </div>

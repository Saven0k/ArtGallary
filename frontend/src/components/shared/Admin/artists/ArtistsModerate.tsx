...
export const ArtistsModerate = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const response = await getModeratedArtists();
            const artistsData = response?.data || [];
            const pendingArtists = artistsData.filter((artist: ArtistUser) => !parseModerate(artist.artistProfile?.moderate));
    ...
    const handleApprove = async (id: number, name: string) => {
        const confirmed = await confirm({ title: "Одобрить художника", message: `Вы уверены, что хотите одобрить художника "${name}"?`, confirmText: "Одобрить", cancelText: "Отмена", type: "info" });
        if (confirmed) {
            try {
                await moderateArtist(id, { moderate: true, moderator_id: Number(user?.id), comment: null, errors: {} });
    ...
    const handleRejectConfirm = async () => {
        ...
        try {
            await moderateArtist(artistToReject!.id, {
                moderate: false, moderator_id: Number(user?.id), comment: rejectComment || null, errors: errorsObject
            });
    ...
    const columns = [
        ...
        { key: 'name', header: 'ФИО', className: 'admin-table__col-name', render: (item: ArtistUser) => `${item.surname} ${item.name} ${item.second_name || ''}` },
        { key: 'email', header: 'Email', className: 'admin-table__col-email' },
        { key: 'phone_number', header: 'Телефон', className: 'admin-table__col-phone' },
        { key: 'artistCity', header: 'Город', className: 'admin-table__col-city', render: (item: ArtistUser) => item.artistProfile?.city?.name_en || '-' },
    ];
    ...
    return (
        <div className="admin-page">
            ...
            {data.length === 0 ? (
                ...
                <AdminTable data={data} columns={columns as any} actions={actions} emptyMessage="Нет художников на модерации" />
            )}
                    ...
                    <div className="admin-view">
                        <div className="admin-view__section">
                            ...
                            <div className="admin-view__grid">
                                <div className="admin-view__item"><span className="admin-view__label">🆔 ID</span><span className="admin-view__value">{selectedArtist.id}</span></div>
                                ...
                                <div className="admin-view__item"><span className="admin-view__label">👤 Имя</span><span className="admin-view__value">{selectedArtist.name}</span></div>
                                <div className="admin-view__item"><span className="admin-view__label">👤 Фамилия</span><span className="admin-view__value">{selectedArtist.surname}</span></div>
                                <div className="admin-view__item"><span className="admin-view__label">👤 Отчество</span><span className="admin-view__value">{selectedArtist.second_name || 'Не указано'}</span></div>
                                <div className="admin-view__item"><span className="admin-view__label">🎭 Роль</span><span className="admin-view__value">{selectedArtist.role}</span></div>
                            </div>
                        </div>
                        ...
                        <div className="admin-view__section">
                            ...
                            <div className="admin-view__grid">
                                <div className="admin-view__item"><span className="admin-view__label">📅 Дата рождения</span><span className="admin-view__value">{selectedArtist.artistProfile?.date_birthday ? new Date(selectedArtist.artistProfile.date_birthday).toLocaleDateString('ru-RU') : '-'}</span></div>
                        ...
                        {selectedArtist.artistProfile?.biography && (
                            <div className="admin-view__section">
                                <h4>📖 Биография</h4>
                                <div className="admin-view__biography">{selectedArtist.artistProfile.biography}</div>
                            </div>
                        )}
                        ...
                        <div className="admin-view__section">
                            <h4>📊 Статистика</h4>
                            <div className="admin-view__grid">
                                <div className="admin-view__item"><span className="admin-view__label">🖼️ Количество работ</span><span className="admin-view__value">{selectedArtist.artistProfile?.artsCount || 0}</span></div>
                                <div className="admin-view__item"><span className="admin-view__label">🏛️ Количество выставок</span><span className="admin-view__value">{selectedArtist.artistProfile?.exhibitionsCount || 0}</span></div>
                            </div>
                        </div>
                        ...
                        {selectedArtist.artistProfile?.arts && selectedArtist.artistProfile.arts.length > 0 && (
                            <div className="admin-view__section">
                                ...
                                <div className="admin-view__arts">
                                    {selectedArtist.artistProfile.arts.map((art: any) => (
                                        <div key={art.id} className="admin-view__art-item">
                                            {art.image_path && (<img src={art.image_path} alt={art.title} className="admin-view__art-image" />)}
                    ...
                    <div className="admin-reject admin-reject--fixed">
                        ...
                        <div className="admin-reject__fields">
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">👤 ФИО</span><button className="admin-reject__add-error" onClick={() => addRejectError('fullName')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.surname} {artistToReject.name} {artistToReject.second_name || ''}</div>
                            </div>
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">👤 Имя</span><button className="admin-reject__add-error" onClick={() => addRejectError('name')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.name}</div>
                            </div>
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">👤 Фамилия</span><button className="admin-reject__add-error" onClick={() => addRejectError('surname')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.surname}</div>
                            </div>
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">👤 Отчество</span><button className="admin-reject__add-error" onClick={() => addRejectError('secondName')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.second_name || 'Не указано'}</div>
                            </div>
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">📧 Email</span><button className="admin-reject__add-error" onClick={() => addRejectError('email')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.email}</div>
                            </div>
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">📱 Телефон</span><button className="admin-reject__add-error" onClick={() => addRejectError('phone')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.phone_number}</div>
                            </div>
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">📅 Дата рождения</span><button className="admin-reject__add-error" onClick={() => addRejectError('birthday')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.artistProfile?.date_birthday ? new Date(artistToReject.artistProfile.date_birthday).toLocaleDateString('ru-RU') : 'Не указано'}</div>
                            </div>
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">🏙️ Город</span><button className="admin-reject__add-error" onClick={() => addRejectError('city')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.artistProfile?.city?.name_en || 'Не указано'}</div>
                            </div>
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">🌍 Страна</span><button className="admin-reject__add-error" onClick={() => addRejectError('country')}>✏️ Добавить замечание</button></div>
                                <div className="admin-reject__field-value">{artistToReject.artistProfile?.country?.name_en || 'Не указано'}</div>
                            </div>
                            {artistToReject.avatar_path && (
                                <div className="admin-reject__field">
                                    <div className="admin-reject__field-header"><span className="admin-reject__field-label">🖼️ Аватар</span><button className="admin-reject__add-error" onClick={() => addRejectError('avatar')}>✏️ Добавить замечание</button></div>
                                    <div className="admin-reject__field-value"><img src={String(artistToReject.avatar_path)} alt="Avatar" className="admin-reject__avatar-preview" /></div>
                                </div>
                            )}
                            <div className="admin-reject__field">
                                <div className="admin-reject__field-header"><span className="admin-reject__field-label">📖 Биография</span><button className="admin-reject__add-error" onClick={() => addRejectError('biography')}>✏️ Добавить замечание</button></div>
                                        ...
                                        <div key={index} className="admin-reject__error-item">
                                            <div className="admin-reject__error-header">
                                                <span className="admin-reject__error-field">
                                                    {error.field === 'fullName' ? 'ФИО' : error.field === 'name' ? 'Имя' : error.field === 'surname' ? 'Фамилия' : error.field === 'secondName' ? 'Отчество' : error.field === 'email' ? 'Email' : error.field === 'phone' ? 'Телефон' : error.field === 'birthday' ? 'Дата рождения' : error.field === 'city' ? 'Город' : error.field === 'country' ? 'Страна' : error.field === 'avatar' ? 'Аватар' : error.field === 'biography' ? 'Биография' : error.field}
                                                </span>
